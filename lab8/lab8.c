...
static count_map_t count_words_parallel(word_t *words, size_t num_words) {
  ...
  for (size_t i = 0; i < THREAD_COUNT; i++) {
    ...
    threads_args[i] = malloc(sizeof(count_thread_args_t));
    threads_args[i]->words = thread_arg_words;
    threads_args[i]->num_words = thread_arg_num_words;
    threads_args[i]->map = &map;
    threads_args[i]->lock = &count_mutex;
