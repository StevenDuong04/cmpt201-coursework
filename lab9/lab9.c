...
port number). Address: 127.0.0.1, Port: 8000
2. Is it UDP or TCP? How do you know?
It is TCP because client calls socket and connect() which UDP does not call
3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code? There is a read() and write()
4. How does the client program end? How can you tell that in the code?
Ends when the read does not read anything because of the if statment > 1 and
...
void *handle_client(void *arg) {
  ...
  while ((bytes = read(cfd, buffer, BUF_SIZE - 1)) > 0) {
    buffer[bytes] = '\0';
    ...
    printf("Client: %d, Total messages: %d, %s", client_id, msg_count, buffer);
  }
